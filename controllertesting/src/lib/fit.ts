...
export function computeVerdict(
  ...
  const ratio =
    width !== null ? Math.round((width / spanMm) * 1000) / 1000 : null;
  const band = fitBand(spanMm, width);
  const layoutMatch =
    model.stickLayout === "either"
      ? true
      : model.stickLayout === stickPreference;
...
export function rankAll(
  ...
  results.sort(
    (a, b) => tierOrder.indexOf(a.verdict) - tierOrder.indexOf(b.verdict),
  );
 ...
 */
export const HAND_REFERENCES: Record<
  HandKey,
  { spanMm: number; note: string }
> = {
  small: {
    spanMm: 150,
    note: "reference hand span used on this page: 150 mm (thumb to pinky, open hand)",
  },
  medium: {
    spanMm: 180,
    note: "reference hand span used on this page: 180 mm (thumb to pinky, open hand)",
  },
  large: {
    spanMm: 205,
    note: "reference hand span used on this page: 205 mm (thumb to pinky, open hand)",
  },
};
