...
*   **Controller Passport & Evidence Report Engine (Marketplace Verification Moat)**: `/report/[uuid]` — Dynamic routing for shareable, versioned diagnostic reports acting as a "Carfax for Controllers." This is the product's primary defensible moat. Users complete an objective hardware test via the `HealthScore.astro` wizard, which automatically generates a self-verifying, checksum-protected payload. The report includes a nuanced result taxonomy (Normal, Anomaly, Hardware Issue), grade badge, and objective hardware metrics (e.g., exact drift %). It outputs a shareable link and a copyable `<iframe>` HTML embed snippet designed specifically for eBay, Reddit, and hardware trading marketplaces to prove controller condition. Explicitly emits `<meta name="robots" content="noindex" />` to prevent crawl budget waste. Report output uses `card-base card-gradient` premium shell.
