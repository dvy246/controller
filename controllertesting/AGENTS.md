...
*   **Known Baseline & QA Audit Status**: `astro check` = 0 errors across all 59 core pages, components, and layouts. Pre-existing 78 errors are strictly isolated inside `src/pages/games/*` canvas `ctx null` checks. Build emits 321 pages.
*   **Color Contrast Compliance**: Verified >7.8:1 contrast in Dark Mode and >8.2:1 in Light Mode across all text layers and status cards (exceeds WCAG AA minimums). CDP light-mode sweep (`sweep.mjs`, 47 routes) = **0 illegible elements**; game pages sweep = 0. Dark-in-both-themes instruments (God-Mode tools + legacy `#*-tester-root` workspaces) carry ID-scoped `[data-theme="light"]` overrides in `src/styles/global.css` — generic light-mode `h1-h4/strong/p` remaps are blocked from overriding their dark palettes. **Mobile layout verified at 390px across 65+ pages** (games, tools, passport, hubs, God-Mode, report, error pages): zero horizontal overflow and zero console errors.
