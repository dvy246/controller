...
```mermaid
graph TD
    A["🧭 Full Diagnostic<br/>(Entry Point)"] --> B["📊 Health Score: 62/100"]
    B --> C{"Issues Found?"}
    C -->|Drift 8.3%| D["🔧 Fix: PS5 Drift Guide"]
    C -->|Trigger 87%| E["🔧 Fix: PS5 Trigger Guide"]
    D --> F["🔄 Re-test After Fix"]
    F --> B
    D --> G["🎯 Optimal Deadzone<br/>for Apex Legends"]
    D --> H["🛒 Buy: Hall Effect<br/>Replacement Stick $8"]
    D --> I["🛒 Upgrade: Best<br/>Controllers for FPS"]
    B --> J["📄 Download<br/>Test Report PDF"]
    B --> K["📈 Save to<br/>Drift Timeline"]
    I --> L["⚔️ Compare:<br/>DualSense vs Xbox"]
    L --> M["🔌 Connect Xbox<br/>to PC Guide"]
```
